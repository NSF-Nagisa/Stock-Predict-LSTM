...
def load_data(data_path, tra_date, val_date, tes_date, seq=2,
                  date_format='%Y-%m-%d'):
    ...
    for date_ind in range(tra_ind, val_ind):
        ...
        if date_ind < seq:
            continue
        for tic_ind in range(len(fnames)):
            if abs(data_EOD[tic_ind][date_ind][-1]) > 1e-8 and \
                ...
                tra_gt[ins_ind, 0] = (data_EOD[tic_ind][date_ind][-1] + 1) / 2
                ins_ind += 1
    ...
    for date_ind in range(val_ind, tes_ind):
        ...
        for tic_ind in range(len(fnames)):
            if abs(data_EOD[tic_ind][date_ind][-1]) > 1e-8 and \
                ...
                val_gt[ins_ind, 0] = (data_EOD[tic_ind][date_ind][-1] + 1) / 2
                ins_ind += 1
    ...
    for date_ind in range(tes_ind, len(trading_dates)):
        ...
        for tic_ind in range(len(fnames)):
            if abs(data_EOD[tic_ind][date_ind][-1]) > 1e-8 and \
                ...
                tes_gt[ins_ind, 0] = (data_EOD[tic_ind][date_ind][-1] + 1) / 2
                ins_ind += 1
