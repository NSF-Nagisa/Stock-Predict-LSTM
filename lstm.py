...
class AWLSTM:
    def __init__(self, data_path, model_path, model_save_path, parameters, steps=1, epochs=50,
                 batch_size=256, gpu=False, tra_date='2018-01-02',
                 val_date='2021-03-01', tes_date='2021-08-02', att=0, hinge=0,
        ...
        self.tes_pv, self.tes_wd, self.tes_gt = load_data(
            self.data_path,
    ...
    def adv_part(self, adv_inputs):
        ...
        if self.att:
            with tf.compat.v1.variable_scope('pre_fc'):
                self.fc_W = tf.compat.v1.get_variable(
                    'weights', dtype=tf.float32,
                    shape=[self.paras['unit'] * 2, 1],
                    initializer=tf.compat.v1.glorot_uniform_initializer()
                )
                self.fc_b = tf.compat.v1.get_variable(
    ...
    def construct_graph(self):
        ...
        with tf.device(device_name):
            ...
            self.wd_var = tf.compat.v1.placeholder(
                tf.float32, [None, self.paras['seq'], 5]
            )

            self.lstm_cell = tf.compat.v1.nn.rnn_cell.LSTMCell(
                self.paras['unit']
            )
            ...
            self.in_lat = tf.compat.v1.layers.dense(
                self.pv_var, units=self.fea_dim,
                activation=tf.nn.tanh, name='in_fc',
                kernel_initializer=tf.compat.v1.glorot_uniform_initializer()
            )

            self.outputs, _ = tf.compat.v1.nn.dynamic_rnn(
                # self.outputs, _ = tf.nn.static_rnn(
            ...
            self.l2_norm = 0
            if self.att:
                with tf.compat.v1.variable_scope('lstm_att') as scope:
                    self.av_W = tf.compat.v1.get_variable(
                        name='att_W', dtype=tf.float32,
                        shape=[self.paras['unit'], self.paras['unit']],
                        initializer=tf.compat.v1.glorot_uniform_initializer()
                    )
                    self.av_b = tf.compat.v1.get_variable(
                        name='att_h', dtype=tf.float32,
                        shape=[self.paras['unit']],
                        initializer=tf.zeros_initializer()
                    )
                    self.av_u = tf.compat.v1.get_variable(
                    ...
                    if self.hinge:
                        self.loss = tf.compat.v1.losses.hinge_loss(self.gt_var, self.pred)
                    else:
                        self.loss = tf.compat.v1.losses.log_loss(self.gt_var, self.pred)
                    ...
                    if self.adv_train:
                        ...
                        if self.hinge:
                            self.adv_loss = tf.compat.v1.losses.hinge_loss(self.gt_var, self.adv_pred)
                        else:
                            self.adv_loss = tf.compat.v1.losses.log_loss(self.gt_var, self.adv_pred)
            else:
                with tf.compat.v1.variable_scope('lstm_att') as scope:
                    print('adversarial scope')
                    # training loss
                    ...
                    if self.adv_train:
                        ...
                        self.adv_pred = self.adv_part(self.adv_pv_var)
                        if self.hinge:
                            self.adv_loss = tf.compat.v1.losses.hinge_loss(self.gt_var,
                                                                 self.adv_pred)
                        else:
                            self.adv_loss = tf.compat.v1.losses.log_loss(self.gt_var,
                                                               self.adv_pred)

            # regularizer
            self.tra_vars = tf.compat.v1.trainable_variables('lstm_att/pre_fc')
